...
authRouter.get('/', async (req: Request, res: Response) => {
  ...
  res.send(`
    ...
    <body>
      ...
      <div class="mcp-url">
        Token: ${user.mcpToken}
        <button class="copy-btn" onclick="navigator.clipboard.writeText('${user.mcpToken}')">Copy</button>
      </div>
...
authRouter.get('/jira/start', async (req: Request, res: Response) => {
  const userId = req.session?.userId;
...
authRouter.get('/slack/start', async (req: Request, res: Response) => {
  const userId = req.session?.userId;
  if (!userId) return res.redirect('/auth');
...
authRouter.get('/github/start', async (req: Request, res: Response) => {
  const userId = req.session?.userId;
  if (!userId) return res.redirect('/auth');
...
authRouter.get('/:service/disconnect', async (req: Request, res: Response) => {
  const userId = req.session?.userId;
  if (!userId) return res.redirect('/auth');
